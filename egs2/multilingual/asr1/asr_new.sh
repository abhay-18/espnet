...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ] && ! [[ " ${skip_stages} " =~ [[:space:]]4[[:space:]] ]]; then
    ...
    # Disable Comments when this is  required, if you're building an LM then sure
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ] && ! [[ " ${skip_stages} " =~ [[:space:]]5[[:space:]] ]]; then
    if [ "${token_type}" = bpe ]; then
        ...
        cat ${bpe_train_text} | cut -f 2- > "${bpedir}"/train.txt
...
if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ] && ! [[ " ${skip_stages} " =~ [[:space:]]11[[:space:]] ]]; then
    ...
    if "${use_lightning}"; then
        ...
        ${python} -u -m espnet2.bin.launch \
            ...
            --multiprocessing_distributed true -- \
            ${python} -u -m espnet2.bin.${asr_task}_train \
                --use_preprocessor true \
                --bpemodel "${bpemodel}" \
                --token_type "${token_type}" \
                --token_list "${token_list}" \
